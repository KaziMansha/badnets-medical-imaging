...
| `src/corruption_utils.py`              | All corruption functions (JPEG, Gaussian/Rician noise, Gibbs ringing, motion ghosting, beam hardening, etc.) and the `CORRUPTION_REGISTRY`      |
| `notebooks/01_preprocessing.ipynb`     | Explores the raw dataset splits and verifies preprocessing                                                                                      |
| `notebooks/02_toy_model.ipynb`         | Defines and trains the toy CNN; runs a single-corruption sweep for quick iteration                                                              |
| `notebooks/03_pretrained_models.ipynb` | Loads pretrained backbones, verifies forward passes, and trains each on clean data                                                              |
| `notebooks/04_model_runs.ipynb`        | Full corruption sweep for pretrained models (ResNet18/50, DenseNet121), including cross-corruption generalization and confusion matrix analysis |
| `notebooks/05_toy_model_runs.ipynb`    | Same corruption sweep for the toy CNN; includes comparison against pretrained results                                                           |
| `run_benchmark.py`                     | One-shot script: installs deps, downloads data, runs toy CNN corruption sweep                                                                   |
| `run_benchmark_pretrained.py`          | One-shot script: trains pretrained models across corruption fractions                                                                           |
| `data/raw/`                            | Raw Training/ and Testing/ image directories (not committed)                                                                                    |
| `data/splits/`                         | Generated train.csv, val.csv, test.csv, and class_map.json                                                                                      |
| `outputs/checkpoints/`                 | Best model checkpoints from notebook 03                                                                                                         |
| `outputs/model_runs/`                  | JSON results and plots from notebooks 04 and 05                                                                                                 |
| `corruption_examples/`                 | Sample corrupted images generated by `corruption_utils.py`                                                                                      |
